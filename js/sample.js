function call_yorkshire() {
  ...
  function ask() {
    ...
    if (retVal != null) {
     // user_review_insert(retVal);
      window.location.href = "loading_sample.html";
    } else {
      return false;
    }
  ...
  function view_change(identification) {
    ...
    $.ajax({
      ...
      success: function (txt) {
        ...
        var p_text0 = document.createTextNode("[요크셔] yy (73.33%) ");
        // var p_text1 = document.createTextNode("[강아지2]  (0.06%) ");
        // var p_text2 = document.createTextNode("[강아지3]  (0.00%)");
        var p_text1 = document.createTextNode("[불독] bull (26.67%) ");
        var p_text2 = document.createTextNode("[래브라도] lab (0.00%)");
  ...
  function popupOpen(src) {
    var url = "demo_sample/index.html?val=" + src.id;    //팝업창 페이지 URL
    var winWidth = 900;
    var winHeight = 600;
  ...
  function remove_item(source) {
    ...
    var target = document.getElementById("test/ml/sample_img/" + img_id).innerHTML = '';
    var dele = "test/ml/sample_img/" + img_id;
    var newimg = document.createElement("img");
  ...
  function change_to_hanguel(input) {
      console.log(input);
    if (input.includes("dog1")) {
      return "[요크셔] ";
    } else if (input.includes("dog2")) {
      return "[불독] ";
    } else {
      return "[래브라도] ";
    }
